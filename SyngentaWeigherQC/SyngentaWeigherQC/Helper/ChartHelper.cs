...
namespace SyngentaWeigherQC.Helper
{
  public class ChartHelper
  {
    ...
    public void AddChartControlDashboard(Chart chartName, List<double> dataY, List<string> dataX, double up2T, double up1T, double target, double lo1T, double lo2T, double max, bool zoom = false)
    {
      try
      {
        if (dataX == null || dataY == null || dataX.Count == 0 || dataY.Count == 0)
        ...
        //double minY = Math.Min(dataY.Min(), lo2T);
        //minY = Math.Min(minY, lo1T);

        //double maxY = Math.Max(dataY.Max(), up2T);
        //maxY = Math.Max(maxY, up1T);


        //if (zoom)
        //{
        //  //maxY = (max < up2T) ? up2T + 5 : up2T * 1.01;
        //  //minY = lo2T - 5;
        //  maxY = Math.Max(up1T, up1T);
        //  maxY = Math.Max(lo2T, lo1T);
        //}
        //else
        //{
        //  //maxY = Math.Max(up1T, up1T);
        ...
        //else
        //{
        //  chartName.ChartAreas[0].AxisY.Maximum = Math.Max(max, up2T) * 1.01;
        //  chartName.ChartAreas[0].AxisY.Minimum = lo2T - 5;
        //}


        //chartName.ChartAreas[0].AxisY.Maximum = maxY + 10;
        //chartName.ChartAreas[0].AxisY.Minimum = minY - 10;
        //chartName.Invalidate();
